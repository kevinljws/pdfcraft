...
type HeaderTableCell = string | { text: string; bold: boolean };
type HeaderTableBody = HeaderTableCell[][];
...
describe("Integration test: snaking columns", function () {
	...
	function getInlineText(item: IntegrationRenderedItem | undefined): string {
		if (!item || !item.inlines) {
			return "";
		}
		return item.inlines.map((i: Inline) => i.text).join("");
	}
	...
	it("should snake content from column 1 to column 2 on the same page", function () {
		...
		var dd = {
			content: [
				{
					columns: [{ text: text, fontSize: 20 }, { text: "" }],
					snakingColumns: true,
				},
			],
		};
		...
		assert.ok(
			uniqueX.length >= 2,
			"Should have content in at least two columns " + JSON.stringify(uniqueX),
		);
	...
	it("should place subsequent content below snaking columns without overlapping", function () {
		...
		assert.ok(
			belowItem.item.y > maxColumnY,
			"Content below should have Y (" +
				belowItem.item.y +
				") greater than max column Y (" +
				maxColumnY +
				")",
		);
	});
	...
	it("should snake content across multiple pages with 3 columns", function () {
		...
		var dd = {
			content: [
				{
					columns: [{ text: text, fontSize: 20 }, { text: "" }, { text: "" }],
					snakingColumns: true,
				},
			],
		};
		...
		assert.ok(
			uniqueXP1.length === 3,
			"Page 1 should have content in 3 columns: " + uniqueXP1.length,
		);
		...
		assert.ok(
			Math.abs(firstItemP2.item.x - uniqueXP1[0]) < 1,
			"Page 2 content should start in first column",
		);
	});
	...
	it("should snake content across multiple pages with 5 columns", function () {
		...
		assert.ok(
			uniqueXP1.length === 5,
			"Page 1 should have content in 5 columns: " + uniqueXP1.length,
		);
	...
	it("should snake content across multiple pages with 2 columns filling more than 1 page (3 cols total)", function () {
		...
		assert.ok(
			Math.abs(uniqueXP2[0] - uniqueXP1[0]) < 1,
			"Page 2 content should start in first column",
		);
	});
	...
	it("should snake content with nested stack structure on single page", function () {
		...
		assert.ok(
			uniqueX.length >= 2,
			"Should have content in at least two columns with stack structure",
		);
	});
	...
	it("should snake content with nested stack structure across multiple pages (Bug Fix Test)", function () {
		...
		assert.ok(
			Math.abs(firstItemX - margins.left) < 1,
	...
	it("should handle unbreakable block that exceeds remaining column height", function () {
		...
		assert.ok(
			uniqueX.length >= 2,
			"Content including unbreakable block should use multiple columns",
		);
	});
	...
	it("should verify Column 2 top Y matches Column 1 top Y (coordinate precision)", function () {
		...
		assert.ok(
			yDiff < 2,
			"Column 1 top Y (" +
				col1FirstItem.item.y +
				") should match Column 2 top Y (" +
				col2FirstItem.item.y +
				")",
		);
	});
	...
	it("should position content after snaking columns below the tallest column", function () {
		...
		assert.ok(
			lastItem.item.y >= maxColumnBottom - 5,
			"Content after (" +
				lastItem.item.y +
				") should be at or below max column bottom (" +
				maxColumnBottom +
				")",
		);
	});
	...
	it("should move unbreakable table to next column if it does not fit in remaining space", function () {
		...
		assert.equal(
			pages.length,
			1,
			"Unbreakable table should fit within snaking columns on one page",
		);
		...
		assert.ok(
			uniqueX.length >= 2,
			"Content should span at least 2 columns (table moved to column 2)",
		);
	});
					...
					columns: [
						{
							stack: [
								{
									text: "Header with nested columns below",
									bold: true,
									fontSize: 14,
									marginBottom: 10,
								},
	...
	it("should support nested snaking columns (inner snaking respected)", function () {
		// Test nested snaking columns where inner columns also snake
		var lines = [];
		for (var i = 1; i <= 90; i++) lines.push("Inner Line " + i);
					...
					columns: [
						{
							stack: [
								{ text: "Outer Column 1 Start" },
								{
									...
									columns: [{ text: innerText, fontSize: 10 }, { text: "" }],
									columnGap: 10,
									snakingColumns: true,
								},
		...
		var innerItems = items.filter((n) => n.item.inlines && n.item.inlines[0].text === "Inner ");
	...
	it("should synchronize all snapshots when page breaks occur in nested structures", function () {
		...
		var margins = testHelper.MARGINS;
		assert.ok(
			Math.abs(firstItemX - margins.left) < 2,
		...
		assert.ok(
			uniqueXP1.length >= 2,
			"Page 1 should use at least 2 column positions, found: " + uniqueXP1.length,
		);
	});
	...
	it("should reflow text when snaking from wide to narrow column", function () {
		...
		col2Items.forEach(function (node) {
			if (node.item.inlines) {
				...
				assert.ok(
					lineWidth <= 105,
					"Line width in narrow column should be <= 100, found: " + lineWidth,
				);
			}
		});
	});
	...
	it("should reset to first column width when breaking to new page in variable width snaking columns", function () {
		...
		assert.ok(
			lineWidth > 150,
			"Page 2 separate column should have reset to wide width (>150), found: " + lineWidth,
		);
	});
	...
	describe("snaking columns nested checks", function () {
		it("should respect left margin when snaking columns break to next page", function () {
			...
			assert.ok(
				Math.abs(firstItemX - expectedX) < 1,
				"Page 2 content should start at left margin + indented margin (" +
					expectedX +
					"), found: " +
					firstItemX,
			);
	...
	describe("snaking columns with table headerRows and keepWithHeaderRows", function () {
		...
		describe("headerRows inside snaking columns", function () {
			it("should repeat header row when table snakes to column 2 on same page", function () {
				var tableBody: HeaderTableBody = [
					[
						{ text: "HDR_A", bold: true },
						{ text: "HDR_B", bold: true },
					],
				];
				for (var i = 1; i <= 40; i++) {
				...
				assert.ok(
					Math.abs(headerPositions[0].y - headerPositions[1].y) < 5,
			...
			it("should repeat header row across multiple pages with snaking columns", function () {
				var tableBody: HeaderTableBody = [
					[
						{ text: "HDR_MULTI_A", bold: true },
						{ text: "HDR_MULTI_B", bold: true },
					],
				];
				for (var i = 1; i <= 200; i++) {
				...
				assert.ok(
					...
					"Header should appear at least once per page (found " +
				...
				var pagesWithHeaders = new Set(
					headerPositions.map(function (p) {
						return p.page;
					}),
				);
				for (var p = 0; p < pages.length; p++) {
					assert.ok(pagesWithHeaders.has(p), "Header should appear on page " + (p + 1));
				}

				// On pages where both columns are used, headers should align at the same Y.
				for (var pageIndex = 0; pageIndex < pages.length; pageIndex++) {
					var pageHeaders = headerPositions.filter(function (pos) {
						return pos.page === pageIndex;
					});
					if (pageHeaders.length >= 2) {
						assert.ok(
							Math.abs(pageHeaders[0].y - pageHeaders[1].y) < 2,
			...
			it("should repeat header row in a 3-column snaking layout", function () {
				...
				var page0Headers = headerPositions.filter(function (p) {
					return p.page === 0;
				});
				assert.ok(
					page0Headers.length >= 2,
		...
		describe("keepWithHeaderRows inside snaking columns", function () {
			...
			it("should not draw duplicate horizontal lines at header-data boundary in continuation columns", function () {
				...
				var hLines: Array<{ y: number; x1: number; x2: number }> = [];
				...
				var duplicates: Array<{ y: number; x1: number; x2: number }> = [];
				...
				assert.equal(
					...
					"Found " +
						duplicates.length +
						" duplicate horizontal line(s): " +
						duplicates
							.map(function (d) {
								return "y=" + d.y + " x1=" + d.x1 + " x2=" + d.x2;
							})
							.join("; "),
				);
			});
		...
		describe("headerRows + keepWithHeaderRows combo in snaking columns", function () {
			it("should support multi-row headers with keepWithHeaderRows in snaking columns", function () {
				...
				assert.ok(
					titlePositions.length >= pages.length,
					"Title header should appear on every page",
				);
				assert.ok(colPositions.length >= pages.length, "Column header should appear on every page");

				// Title and column headers should appear at same x positions
				// (matching column structure)
				pages.forEach(function (page, pi) {
					var pageTitles = titlePositions.filter(function (p) {
						return p.page === pi;
					});
					var pageCols = colPositions.filter(function (p) {
						return p.page === pi;
					});
	...
	describe("snaking columns with table nodes", function () {
		...
		it("should snake a real table node from column 1 to column 2 on the same page", function () {
			...
			assert.ok(
				uniqueX.length >= 2,
				"Table should snake to column 2, got X positions: " + JSON.stringify(uniqueX),
			);
		});
		...
		it("should preserve table borders in both columns", function () {
			...
			assert.ok(
				maxX - minX > 100,
				"Table borders should span across both columns, got range: " + minX + " to " + maxX,
			);
		});
		...
		it("should handle large table spanning multiple pages with snaking columns", function () {
			...
			assert.ok(
				pages.length <= 5,
				"With snaking, should use far fewer pages than single-column (got " + pages.length + ")",
			);
			...
			for (var p = 0; p < pages.length - 1; p++) {
				...
				if (lineItems.length > 10) {
					assert.ok(
						uniqueX.length >= 2,
						"Page " + (p + 1) + " should have content in at least 2 columns",
					);
				}
			}
		});
		...
		it("should draw top border at continuation point after column and page breaks", function () {
			...
			pages.forEach(function (page, pi) {
				...
				var col2Lines = hLines.filter(function (v) {
					return v.item.x1 >= 300;
				});

				if (col2Lines.length > 0) {
					var col1Lines = hLines.filter(function (v) {
						return v.item.x1 < 300;
					});
					var col1TopY = Math.min.apply(
						null,
						col1Lines.map(function (v) {
							return v.item.y1;
						}),
					);
					var col2TopY = Math.min.apply(
						null,
						col2Lines.map(function (v) {
							return v.item.y1;
						}),
					);
