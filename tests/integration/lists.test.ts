...
describe("Integration test: lists", function () {
	...
	function getBulletListLine(pages: IntegrationPage[], options: ListLineOptions) {
		...
		return {
			bullet: bullet?.item as IntegrationRenderedItem,
			content: content?.item as IntegrationRenderedItem,
		};
	}
	...
	it("renders a ordered list", function () {
		...
		assert.equal(
			item3.content.inlines.map((inline) => inline.text).join(""),
			"item 3 - Paleo American Apparel ",
		);
	...
	it("renders nested lists", function () {
		var dd = {
			content: {
				ol: ["item 1", ["item 2", { ol: ["subitem 1", "subitem 2"] }], "item 3"],
			},
		};
