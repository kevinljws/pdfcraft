...
describe("Integration test: background", function () {
	...
	it("renders on every page", function () {
		...
		assert.equal(
			backgroundPage1.inlines.map((node) => node.text).join(""),
			"Background paragraph on page 1",
		);
		...
		assert.equal(
			backgroundPage2.inlines.map((node) => node.text).join(""),
			"Background paragraph on page 2",
		);
