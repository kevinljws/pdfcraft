...
class DocumentContextColumns {
	constructor(private readonly state: DocumentContextState) {}
	...
	resetMarginXTopParent(): void {
		this.state.marginXTopParent = null;
	}

	beginColumn(
		width: number = this.state.availableWidth,
		offset = 0,
	...
	calculateBottomMost(destination: ContextSnapshot, endingCell: ColumnEndingCell | null): void {
		if (endingCell) {
			...
			destination.bottomMost = bottomMostContext(this.state, destination.bottomMost);
		}
	}
