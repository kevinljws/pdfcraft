...
import BaseLine from "../line.ts";
import type { Inline } from "../../types/internal.ts";

type InlineFixture = Pick<Inline, "width"> & Partial<Inline>;

class Line extends BaseLine {
	override addInline(inline: InlineFixture): void {
		super.addInline(inline as Inline);
	}
...
describe("Line", function () {
	...
	describe("hasEnoughSpaceForInline", function () {
		...
		it("should keep noNewLine inline groups together", function () {
			var line = new Line(100);
			line.addInline({ width: 40 });

			assert(
				!line.hasEnoughSpaceForInline({ width: 20, noNewLine: true }, [
					{ width: 25, noNewLine: true, trailingCut: 10 },
					{ width: 21, trailingCut: 5 },
				]),
			);
			assert(
				line.hasEnoughSpaceForInline({ width: 20, noNewLine: true }, [
