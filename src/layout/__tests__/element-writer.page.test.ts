...
describe("PageElementWriter", function () {
	...
	function buildSvg(height: number): PdfNode {
		return {
	...
	function createRepeatable(marker: unknown, height: number): RepeatableFixture {
		var rep: RepeatableFixture = { items: [], height, insertedOnPages: [] };
	...
	describe("addSVG", function () {
		it("moves an SVG to a new page under the same overflow rule as an image", function () {
			pew.addLine(buildLine(900));

			const position = pew.addSVG(buildSvg(300));

			assert.equal(ctx.pages.length, 2);
			assert.equal(ctx.pages[0].items.length, 1);
			assert.equal(ctx.pages[1].items[0].type, "svg");
			assert.equal(position && position.pageNumber, 2);
			assert.equal(position && position.top, MARGINS.top);
		});

		it("renders an oversized SVG on an otherwise empty page", function () {
			const position = pew.addSVG(buildSvg(2000));
	...
	describe("beginUnbreakableBlock", function () {
		it("should begin a new transaction only once", function () {
			...
			var transactionCtx = pew.context();

			pew.beginUnbreakableBlock();

			assert.equal(transactionCtx, pew.context());
		});
	...
	describe("commitUnbreakableBlock", function () {
		...
		it("uses vector bounds when committing a block", function () {
			pew.beginUnbreakableBlock();
	...
	describe("currentBlockToRepeatable", function () {
		...
		it("uses vector bounds as the repeatable height", function () {
			pew.beginUnbreakableBlock();
			pew.addVector({ type: "line", x1: 0, y1: 0, x2: 20, y2: 80, lineWidth: 2 });

			const repeatable = pew.currentBlockToRepeatable();

			assert.equal(repeatable.height, 81);
		});
	});
	...
	describe("creating a new page", function () {
		it("should add a repeatable fragment to the top", function () {
			...
			uCtx.pages[0].items.forEach(function (item) {
				(item.item as LineFixture).marker = "rep";
			});
			var rep = pew.currentBlockToRepeatable();
			pew.pushToRepeatables(rep);
			pew.commitUnbreakableBlock();
			...
			assert.equal(ctx.pages.length, 2);
			assert.equal(ctx.pages[0].items.length, 10);
			assert.equal(itemAt(0, 6).marker, "rep");
			assert.equal(itemAt(0, 7).marker, "rep");
			assert.equal(itemAt(0, 8).marker, "rep");
			assert(!itemAt(0, 9).marker);
		...
		it("should use existing page", function () {
			addOneTenthLines(1);
			ctx.pages.push({ items: [], pageSize: pageSize } as unknown as PdfPage);
			(ctx as unknown as { availableWidth: unknown }).availableWidth = "garbage";
			(ctx as unknown as { availableHeight: unknown }).availableHeight = "garbage";
