...
import type { DecorationGroup } from "./text.types";
	...
	for (let i = 0, l = line.inlines.length; i < l; i++) {
		...
		for (let ii = 0, ll = decoration.length; ii < ll; ii++) {
			...
			if (
				...
				thickness !== currentGroup.decorationThickness
			) {
				currentGroup = {
					line: line,
...
class TextDecorator {
	...
	_drawDecoration(group: DecorationGroup, x: number, y: number): void {
		const maxInline = (): Inline => {
			return group.inlines.reduce((largest, inline) =>
				inline.fontSize > largest.fontSize ? inline : largest,
			);
		};
		...
		if (group.decorationStyle === "double") {
			...
			for (let i = 0; i < nbWaves; i++) {
				this.pdfDocument
					.bezierCurveTo(rwx + sh, y - sv, rwx + sh * 2, y - sv, rwx + sh * 3, y)
