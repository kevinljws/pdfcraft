			...
			stack: [
				{ text: "Card " + i, fontSize: 14, bold: true, margin: [0, 0, 0, 5] },
				{
					text:
						...
						i +
						". All content in this card stays together as an unbreakable unit.",
					fontSize: 10,
				},
...
var docDefinition = {
	content: [
		...
		{
			text: "The snakingColumns: true property allows content to flow from one column to the next on the same page, similar to newspaper layouts.",
			style: "description",
		},
		" ",
		...
		{
			text: "This text correctly appears after the snaking columns section, demonstrating that the layout engine properly tracks the bottom position of content spanning multiple columns.",
			style: "description",
		},
		" ",
		...
		{
			text: "Without snakingColumns, each column is independent. Content in column 1 stays in column 1, and content in column 2 stays in column 2.",
			style: "description",
		},
		" ",
		{
			columns: [
				{ text: loremIpsum },
				{
					text: "This is separate content in column 2. It does not receive overflow from column 1.",
					bold: true,
				},
			],
			columnGap: 20,
		...
		{
			text: "Content starts in the narrow column and overflows to the wider column.",
			style: "description",
		},
		" ",
		...
		{
			text: "Example 2: Wide Left Column (350pt) + Narrow Right Column (*)",
			style: "subheader",
			pageBreak: "before",
		},
		{
			text: "Content starts in the wide column and overflows to the remaining space.",
			style: "description",
		},
		" ",
		...
		{
			text: "Content flows: Col 1 -> 2 -> 3 -> 4 -> 5 -> Next Page Col 1...",
			style: "description",
		},
		" ",
		...
		{
			text: "Example 5: Landscape Orientation",
			style: "subheader",
			pageBreak: "before",
		...
		{
			text: "Snaking Columns with Different Column Gaps",
			style: "header",
			pageBreak: "before",
		...
		{
			text: "This example demonstrates nested regular columns inside a snaking layout.",
			style: "description",
		},
		" ",
			...
			columns: [
				{
					stack: [
						...
						{
							text: Array.from({ length: 80 }, function (_, i) {
								return (
									"Note " +
									(i + 1) +
									": Inventory check completed for item batch #" +
									(1000 + i) +
									"."
								);
							}).join("\n"),
			...
			columns: [
				{
					stack: [
							...
							columns: [
								{
									width: "50%",
									text:
										"Inner Column 1 (Long content to force overflow):\n" +
										"Inner Line\n".repeat(20),
									fontSize: 10,
								},
								{
						...
						{
							text: Array.from({ length: 100 }, function (_, i) {
								return (
									"Q" +
									((i % 4) + 1) +
									" FY" +
		...
		{
			text: "Snaking Columns Stack Text Example",
			style: "header",
			margin: [0, 0, 0, 20],
			pageBreak: "before",
		},
		...
		{
			text: "Each card is marked as unbreakable, ensuring it stays together when flowing between columns.",
			style: "description",
		},
		" ",
		...
		{
			text: "This example demonstrates how SVG graphics render within snaking columns.",
			style: "description",
		},
		" ",
			...
			columns: [
				{
					stack: [
						...
						{
							text: "Below is an SVG graphic that fits within the column width.",
							marginBottom: 10,
						},
						...
						{
							text: "Success, Warning, and Error status indicators shown above.",
							fontSize: 9,
							marginBottom: 15,
						},
		...
		{
			text: "All SVG elements rendered correctly within the snaking layout.",
			style: "description",
		},
	],
	styles: {
