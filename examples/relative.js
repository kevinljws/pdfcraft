...
function buildXAxis() {
	...
	chartLines.push(
		horizontalLine(left + yAxisWidth - 1, top + height - xAxisHeight, width - yAxisWidth + 1),
	);

	xTicks.forEach(function (tick) {
		chartLines.push(
			verticalLine(left + yAxisWidth + tick.x - 0.5, top + height - xAxisHeight, tickSize),
		);
		chartText.push({
			text: tick.t,
			fontSize: 8,
...
function buildYAxis() {
	...
	yTicks.forEach(function (tick) {
		...
		chartText.push({
			text: tick.t,
			fontSize: 8,
			relativePosition: { x: left + yAxisWidth - tickSize - 8, y: top + tick.y - 5 },
		});
	});
}
